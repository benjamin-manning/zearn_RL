...
data {
  ...
  int<lower=1, upper=3> choice[N, T]; //original: int<lower=1, upper=4> choice[N, T];
  real outcome[N, T];  // no lower and upper bounds
}
transformed data {
  vector[3] initV;  // initial values for EV
...
transformed parameters {
  ...
  vector<lower=0, upper=3>[N] tau; //trying tau at 3 for second try

  for (i in 1:N) {
    A[i]   = Phi_approx(mu_pr[1]  + sigma[1]  * A_pr[i]); //approx Normal CDF + noise
